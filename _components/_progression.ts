...
export function getWeaponUpgrades(scene: IGameScene): any[] {
  ...
  const paths: Record<WeaponType, Step[]> = {
    shotgun: [
      { desc: '+2 pellets  ·  −50ms cooldown',             apply: () => { scene.extraBullets += 2; scene.flatWeaponShootRateReductions['shotgun'] = (scene.flatWeaponShootRateReductions['shotgun'] ?? 0) + 50; scene.recalculateStats() } },
      { desc: '+30% damage  ·  +50px range',               apply: () => { scene.bonusWeaponDmg['shotgun'] = (scene.bonusWeaponDmg['shotgun'] ?? 0) + 0.3; scene.shotgunRange += 50; scene.recalculateStats() } },
      { desc: 'Unlock rear shot',                          apply: () => { scene.weaponRearShot['shotgun'] = true } },
      { desc: '+2 pellets  ·  +30% damage',                apply: () => { scene.extraBullets += 2; scene.bonusWeaponDmg['shotgun'] = (scene.bonusWeaponDmg['shotgun'] ?? 0) + 0.3; scene.recalculateStats() } },
      { desc: '−80ms cooldown  ·  +60px range',            apply: () => { scene.flatWeaponShootRateReductions['shotgun'] = (scene.flatWeaponShootRateReductions['shotgun'] ?? 0) + 80; scene.shotgunRange += 60; scene.recalculateStats() } },
      { desc: '+40% damage  ·  +2 pellets',                apply: () => { scene.bonusWeaponDmg['shotgun'] = (scene.bonusWeaponDmg['shotgun'] ?? 0) + 0.4; scene.extraBullets += 2; scene.recalculateStats() } },
      { desc: '−80ms cooldown  ·  +60px range',            apply: () => { scene.flatWeaponShootRateReductions['shotgun'] = (scene.flatWeaponShootRateReductions['shotgun'] ?? 0) + 80; scene.shotgunRange += 60; scene.recalculateStats() } },
      { desc: '+60% damage  ·  +4 pellets  ·  −100ms',     apply: () => { scene.bonusWeaponDmg['shotgun'] = (scene.bonusWeaponDmg['shotgun'] ?? 0) + 0.6; scene.extraBullets += 4; scene.flatWeaponShootRateReductions['shotgun'] = (scene.flatWeaponShootRateReductions['shotgun'] ?? 0) + 100; scene.recalculateStats() } },
    ],
    sniper: [
      { desc: '+1 pierce  ·  +50% damage',                 apply: () => { scene.pierceCount++; scene.bonusWeaponDmg['sniper'] = (scene.bonusWeaponDmg['sniper'] ?? 0) + 0.5; scene.recalculateStats() } },
      { desc: '−250ms cooldown  ·  +30% bullet speed',     apply: () => { scene.flatWeaponShootRateReductions['sniper'] = (scene.flatWeaponShootRateReductions['sniper'] ?? 0) + 250; scene.bonusWeaponBulletSpd['sniper'] = (scene.bonusWeaponBulletSpd['sniper'] ?? 0) + 0.3; scene.recalculateStats() } },
      { desc: 'Rear shot  ·  +50% damage',                 apply: () => { scene.weaponRearShot['sniper'] = true; scene.bonusWeaponDmg['sniper'] = (scene.bonusWeaponDmg['sniper'] ?? 0) + 0.5; scene.recalculateStats() } },
      { desc: '+2 pierce  ·  −200ms cooldown',             apply: () => { scene.pierceCount += 2; scene.flatWeaponShootRateReductions['sniper'] = (scene.flatWeaponShootRateReductions['sniper'] ?? 0) + 200; scene.recalculateStats() } },
      { desc: '+70% damage  ·  +30% bullet speed',         apply: () => { scene.bonusWeaponDmg['sniper'] = (scene.bonusWeaponDmg['sniper'] ?? 0) + 0.7; scene.bonusWeaponBulletSpd['sniper'] = (scene.bonusWeaponBulletSpd['sniper'] ?? 0) + 0.3; scene.recalculateStats() } },
      { desc: '+2 pierce  ·  −200ms cooldown',             apply: () => { scene.pierceCount += 2; scene.flatWeaponShootRateReductions['sniper'] = (scene.flatWeaponShootRateReductions['sniper'] ?? 0) + 200; scene.recalculateStats() } },
      { desc: '+80% damage  ·  +30% bullet speed',         apply: () => { scene.bonusWeaponDmg['sniper'] = (scene.bonusWeaponDmg['sniper'] ?? 0) + 0.8; scene.bonusWeaponBulletSpd['sniper'] = (scene.bonusWeaponBulletSpd['sniper'] ?? 0) + 0.3; scene.recalculateStats() } },
