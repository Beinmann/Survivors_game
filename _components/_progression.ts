...
export function getWeaponUpgrades(scene: IGameScene): any[] {
  ...
  const paths: Record<WeaponType, Step[]> = {
    ...
    scythes: [
      ...
      { desc: '+30% damage  ·  +20px radius',              icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['scythes'] = (scene.bonusWeaponDmg['scythes'] ?? 0) + 0.3; scene.scythesRadius += 20; scene.recalculateStats() } },
      { desc: '+1 blade',                                  icon: 'wico_scythes', apply: () => { scene.scythesCount++; scene.recalculateStats() } },
      { desc: '+40% damage  ·  +20px radius',              icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['scythes'] = (scene.bonusWeaponDmg['scythes'] ?? 0) + 0.4; scene.scythesRadius += 20; scene.recalculateStats() } },
      ...
      { desc: '+1 blade',                                  icon: 'wico_scythes', apply: () => { scene.scythesCount++; scene.recalculateStats() } },
      { desc: '+60% damage  ·  +30px radius',              icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['scythes'] = (scene.bonusWeaponDmg['scythes'] ?? 0) + 0.6; scene.scythesRadius += 30; scene.recalculateStats() } },
