...
export function getWeaponUpgrades(scene: IGameScene): any[] {
  ...
  const paths: Record<WeaponType, Step[]> = {
    ...
    aura: [
      ...
      { desc: '+8px radius  ·  −50ms cooldown',            icon: 'ico_radius', apply: () => { scene.auraRadius += 8; scene.flatWeaponShootRateReductions['aura'] = (scene.flatWeaponShootRateReductions['aura'] ?? 0) + 50; scene.recalculateStats() } },
      { desc: '+25% damage  ·  +8px radius',               icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['aura'] = (scene.bonusWeaponDmg['aura'] ?? 0) + 0.25; scene.auraRadius += 8; scene.recalculateStats() } },
      { desc: '−80ms cooldown  ·  +12px radius',           icon: 'ico_cooldown', apply: () => { scene.flatWeaponShootRateReductions['aura'] = (scene.flatWeaponShootRateReductions['aura'] ?? 0) + 80; scene.auraRadius += 12; scene.recalculateStats() } },
      { desc: '+30% damage  ·  +12px radius',              icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['aura'] = (scene.bonusWeaponDmg['aura'] ?? 0) + 0.30; scene.auraRadius += 12; scene.recalculateStats() } },
      { desc: '−80ms cooldown  ·  +16px radius',           icon: 'ico_cooldown', apply: () => { scene.flatWeaponShootRateReductions['aura'] = (scene.flatWeaponShootRateReductions['aura'] ?? 0) + 80; scene.auraRadius += 16; scene.recalculateStats() } },
      { desc: '+40% damage  ·  +16px radius',              icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['aura'] = (scene.bonusWeaponDmg['aura'] ?? 0) + 0.40; scene.auraRadius += 16; scene.recalculateStats() } },
      { desc: '+50% damage  ·  +20px radius  ·  −80ms',    icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['aura'] = (scene.bonusWeaponDmg['aura'] ?? 0) + 0.50; scene.auraRadius += 20; scene.flatWeaponShootRateReductions['aura'] = (scene.flatWeaponShootRateReductions['aura'] ?? 0) + 80; scene.recalculateStats() } },
    ],
    machinegun: [
      { desc: '+30% damage  ·  −20ms cooldown',            icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['machinegun'] = (scene.bonusWeaponDmg['machinegun'] ?? 0) + 0.3; scene.flatWeaponShootRateReductions['machinegun'] = (scene.flatWeaponShootRateReductions['machinegun'] ?? 0) + 20; scene.recalculateStats() } },
      { desc: '+30% damage  ·  −20ms cooldown',            icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['machinegun'] = (scene.bonusWeaponDmg['machinegun'] ?? 0) + 0.3; scene.flatWeaponShootRateReductions['machinegun'] = (scene.flatWeaponShootRateReductions['machinegun'] ?? 0) + 20; scene.recalculateStats() } },
      { desc: '20% crit chance — crits deal 3× damage',    icon: 'ico_crit', apply: () => { scene.machineGunCritChance = 0.2 } },
      { desc: '+30% damage  ·  −20ms cooldown',            icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['machinegun'] = (scene.bonusWeaponDmg['machinegun'] ?? 0) + 0.3; scene.flatWeaponShootRateReductions['machinegun'] = (scene.flatWeaponShootRateReductions['machinegun'] ?? 0) + 20; scene.recalculateStats() } },
      { desc: '+30% damage  ·  −20ms cooldown',            icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['machinegun'] = (scene.bonusWeaponDmg['machinegun'] ?? 0) + 0.3; scene.flatWeaponShootRateReductions['machinegun'] = (scene.flatWeaponShootRateReductions['machinegun'] ?? 0) + 20; scene.recalculateStats() } },
      { desc: '+35% damage  ·  −25ms cooldown',            icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['machinegun'] = (scene.bonusWeaponDmg['machinegun'] ?? 0) + 0.35; scene.flatWeaponShootRateReductions['machinegun'] = (scene.flatWeaponShootRateReductions['machinegun'] ?? 0) + 25; scene.recalculateStats() } },
      { desc: '+40% damage  ·  −30ms cooldown',            icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['machinegun'] = (scene.bonusWeaponDmg['machinegun'] ?? 0) + 0.4; scene.flatWeaponShootRateReductions['machinegun'] = (scene.flatWeaponShootRateReductions['machinegun'] ?? 0) + 30; scene.recalculateStats() } },
      { desc: '+1 bullet per shot  ·  +40% damage',        icon: 'ico_pellets', apply: () => { scene.machineGunBullets++; scene.bonusWeaponDmg['machinegun'] = (scene.bonusWeaponDmg['machinegun'] ?? 0) + 0.4; scene.recalculateStats() } },
    ],
    scythes: [
    ...
    blackhole: [
      ...
      { desc: 'Wider Influence — +40px outer  ·  +20px mid',                  icon: 'ico_radius', apply: () => { scene.blackholeOuterRadius += 40; scene.blackholeMidRadius += 20 } },
      { desc: 'Crushing Grip — +50% damage  ·  +30 mid pull',                 icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['blackhole'] = (scene.bonusWeaponDmg['blackhole'] ?? 0) + 0.5; scene.blackholeMidPull += 30; scene.recalculateStats() } },
      { desc: 'Heavier Collapse — +60% damage  ·  +500ms duration',           icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['blackhole'] = (scene.bonusWeaponDmg['blackhole'] ?? 0) + 0.6; scene.blackholeDuration += 500; scene.recalculateStats() } },
      { desc: 'Singularity — +20px core  ·  +60 core pull',                   icon: 'wico_blackhole', apply: () => { scene.blackholeCoreRadius += 20; scene.blackholeCorePull += 60 } },
      { desc: 'Dark Gravity — +60px outer  ·  +20 outer pull  ·  +20 mid pull', icon: 'ico_radius', apply: () => { scene.blackholeOuterRadius += 60; scene.blackholeOuterPull += 20; scene.blackholeMidPull += 20 } },
      { desc: 'Devourer — −1500ms cooldown  ·  +500ms duration',              icon: 'ico_cooldown', apply: () => { scene.flatWeaponShootRateReductions['blackhole'] = (scene.flatWeaponShootRateReductions['blackhole'] ?? 0) + 1500; scene.blackholeDuration += 500; scene.recalculateStats() } },
    ...
    cleave: [
      { desc: '+40% damage  ·  +12px radius',              icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['cleave'] = (scene.bonusWeaponDmg['cleave'] ?? 0) + 0.4; scene.cleaveRadius += 12; scene.recalculateStats() } },
      { desc: '−250ms cooldown  ·  +15° arc',              icon: 'ico_cooldown', apply: () => { scene.flatWeaponShootRateReductions['cleave'] = (scene.flatWeaponShootRateReductions['cleave'] ?? 0) + 250; scene.cleaveArc += (15 * Math.PI) / 180; scene.recalculateStats() } },
      ...
      { desc: '+50% damage  ·  +15px radius',              icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['cleave'] = (scene.bonusWeaponDmg['cleave'] ?? 0) + 0.5; scene.cleaveRadius += 15; scene.recalculateStats() } },
      { desc: '−250ms cooldown  ·  +15° arc',              icon: 'ico_cooldown', apply: () => { scene.flatWeaponShootRateReductions['cleave'] = (scene.flatWeaponShootRateReductions['cleave'] ?? 0) + 250; scene.cleaveArc += (15 * Math.PI) / 180; scene.recalculateStats() } },
      { desc: '+1 slash  ·  +15° arc',                     icon: 'wico_cleave', apply: () => { scene.cleaveCount++; scene.cleaveArc += (15 * Math.PI) / 180 } },
      { desc: '+70% damage  ·  +18px radius',              icon: 'ico_damage', apply: () => { scene.bonusWeaponDmg['cleave'] = (scene.bonusWeaponDmg['cleave'] ?? 0) + 0.7; scene.cleaveRadius += 18; scene.recalculateStats() } },
