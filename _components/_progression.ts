...
export function getWeaponUpgrades(scene: IGameScene): any[] {
  ...
  const paths: Record<WeaponType, Step[]> = {
    shotgun: [
      { desc: '+2 pellets  ·  −50ms cooldown',             icon: 'ico_pellets',     apply: () => { scene.extraBullets += 2; scene.weaponShootRates['shotgun'] = Math.max(100, (scene.weaponShootRates['shotgun'] ?? 550) - 50) } },
      { desc: '+30% damage  ·  +50px range',               icon: 'ico_damage',      apply: () => { scene.shotgunDmg = Math.round(scene.shotgunDmg * 1.3); scene.shotgunRange += 50 } },
      { desc: 'Unlock rear shot',                          icon: 'ico_rearshot',    apply: () => { scene.weaponRearShot['shotgun'] = true } },
      { desc: '+2 pellets  ·  +30% damage',                icon: 'ico_pellets',     apply: () => { scene.extraBullets += 2; scene.shotgunDmg = Math.round(scene.shotgunDmg * 1.3) } },
      { desc: '−80ms cooldown  ·  +60px range',            icon: 'ico_cooldown',    apply: () => { scene.weaponShootRates['shotgun'] = Math.max(100, (scene.weaponShootRates['shotgun'] ?? 550) - 80); scene.shotgunRange += 60 } },
      { desc: '+40% damage  ·  +2 pellets',                icon: 'ico_damage',      apply: () => { scene.shotgunDmg = Math.round(scene.shotgunDmg * 1.4); scene.extraBullets += 2 } },
      { desc: '−80ms cooldown  ·  +60px range',            icon: 'ico_range',       apply: () => { scene.weaponShootRates['shotgun'] = Math.max(100, (scene.weaponShootRates['shotgun'] ?? 550) - 80); scene.shotgunRange += 60 } },
      { desc: '+60% damage  ·  +4 pellets  ·  −100ms',     icon: 'ico_pellets',     apply: () => { scene.shotgunDmg = Math.round(scene.shotgunDmg * 1.6); scene.extraBullets += 4; scene.weaponShootRates['shotgun'] = Math.max(100, (scene.weaponShootRates['shotgun'] ?? 550) - 100) } },
    ],
    sniper: [
      { desc: '+1 pierce  ·  +50% damage',                 icon: 'ico_pierce',      apply: () => { scene.pierceCount++; scene.sniperDmg = Math.round(scene.sniperDmg * 1.5) } },
      { desc: '−250ms cooldown  ·  +30% bullet speed',     icon: 'ico_cooldown',    apply: () => { scene.weaponShootRates['sniper'] = Math.max(300, (scene.weaponShootRates['sniper'] ?? 1400) - 250); scene.weaponBulletSpd['sniper'] = Math.round((scene.weaponBulletSpd['sniper'] ?? 680) * 1.3) } },
      { desc: 'Rear shot  ·  +50% damage',                 icon: 'ico_rearshot',    apply: () => { scene.weaponRearShot['sniper'] = true; scene.sniperDmg = Math.round(scene.sniperDmg * 1.5) } },
      { desc: '+2 pierce  ·  −200ms cooldown',             icon: 'ico_pierce',      apply: () => { scene.pierceCount += 2; scene.weaponShootRates['sniper'] = Math.max(300, (scene.weaponShootRates['sniper'] ?? 1400) - 200) } },
      { desc: '+70% damage  ·  +30% bullet speed',         icon: 'ico_bulletspeed', apply: () => { scene.sniperDmg = Math.round(scene.sniperDmg * 1.7); scene.weaponBulletSpd['sniper'] = Math.round((scene.weaponBulletSpd['sniper'] ?? 680) * 1.3) } },
      { desc: '+2 pierce  ·  −200ms cooldown',             icon: 'ico_pierce',      apply: () => { scene.pierceCount += 2; scene.weaponShootRates['sniper'] = Math.max(300, (scene.weaponShootRates['sniper'] ?? 1400) - 200) } },
      { desc: '+80% damage  ·  +30% bullet speed',         icon: 'ico_damage',      apply: () => { scene.sniperDmg = Math.round(scene.sniperDmg * 1.8); scene.weaponBulletSpd['sniper'] = Math.round((scene.weaponBulletSpd['sniper'] ?? 680) * 1.3) } },
      { desc: '+3 pierce  ·  +100% damage  ·  −200ms',     icon: 'ico_pierce',      apply: () => { scene.pierceCount += 3; scene.sniperDmg = Math.round(scene.sniperDmg * 2.0); scene.weaponShootRates['sniper'] = Math.max(300, (scene.weaponShootRates['sniper'] ?? 1400) - 200) } },
    ],
    aura: [
      { desc: '+30% damage',                               icon: 'ico_damage',      apply: () => { scene.auraDmg = Math.round(scene.auraDmg * 1.3) } },
      { desc: '+25px radius  ·  −80ms cooldown',           icon: 'ico_radius',      apply: () => { scene.auraRadius += 25; scene.weaponShootRates['aura'] = Math.max(100, (scene.weaponShootRates['aura'] ?? 500) - 80) } },
      { desc: '+50% damage  ·  +25px radius',              icon: 'ico_damage',      apply: () => { scene.auraDmg = Math.round(scene.auraDmg * 1.5); scene.auraRadius += 25 } },
      { desc: '−100ms cooldown  ·  +30px radius',          icon: 'ico_radius',      apply: () => { scene.weaponShootRates['aura'] = Math.max(100, (scene.weaponShootRates['aura'] ?? 500) - 100); scene.auraRadius += 30 } },
      { desc: '+60% damage  ·  +30px radius',              icon: 'ico_damage',      apply: () => { scene.auraDmg = Math.round(scene.auraDmg * 1.6); scene.auraRadius += 30 } },
      { desc: '−100ms cooldown  ·  +35px radius',          icon: 'ico_radius',      apply: () => { scene.weaponShootRates['aura'] = Math.max(100, (scene.weaponShootRates['aura'] ?? 500) - 100); scene.auraRadius += 35 } },
      { desc: '+80% damage  ·  +35px radius',              icon: 'ico_damage',      apply: () => { scene.auraDmg = Math.round(scene.auraDmg * 1.8); scene.auraRadius += 35 } },
      { desc: '+100% damage  ·  +50px radius  ·  −100ms',  icon: 'ico_radius',      apply: () => { scene.auraDmg = Math.round(scene.auraDmg * 2.0); scene.auraRadius += 50; scene.weaponShootRates['aura'] = Math.max(100, (scene.weaponShootRates['aura'] ?? 500) - 100) } },
    ],
    machinegun: [
      { desc: '+40% damage  ·  −15ms cooldown',            icon: 'ico_damage',      apply: () => { scene.machineGunDmg = Math.round(scene.machineGunDmg * 1.4); scene.weaponShootRates['machinegun'] = Math.max(50, (scene.weaponShootRates['machinegun'] ?? 100) - 15) } },
      { desc: '+40% damage  ·  −15ms cooldown',            icon: 'ico_damage',      apply: () => { scene.machineGunDmg = Math.round(scene.machineGunDmg * 1.4); scene.weaponShootRates['machinegun'] = Math.max(50, (scene.weaponShootRates['machinegun'] ?? 100) - 15) } },
      { desc: 'Piercing rounds — bullets pass through 1 enemy', icon: 'ico_pierce', apply: () => { scene.machineGunPierce = true } },
      { desc: '+40% damage  ·  −15ms cooldown',            icon: 'ico_cooldown',    apply: () => { scene.machineGunDmg = Math.round(scene.machineGunDmg * 1.4); scene.weaponShootRates['machinegun'] = Math.max(50, (scene.weaponShootRates['machinegun'] ?? 100) - 15) } },
      { desc: 'Burst fire — 2 bullets per shot',           icon: 'ico_burst',       apply: () => { scene.machineGunBurst = 2 } },
      { desc: 'Rear shot  ·  +30% damage',                 icon: 'ico_rearshot',    apply: () => { scene.weaponRearShot['machinegun'] = true; scene.machineGunDmg = Math.round(scene.machineGunDmg * 1.3) } },
      { desc: '3-round burst  ·  −20ms cooldown',          icon: 'ico_burst',       apply: () => { scene.machineGunBurst = 3; scene.weaponShootRates['machinegun'] = Math.max(50, (scene.weaponShootRates['machinegun'] ?? 100) - 20) } },
      { desc: '+60% damage  ·  −20ms cooldown',            icon: 'ico_damage',      apply: () => { scene.machineGunDmg = Math.round(scene.machineGunDmg * 1.6); scene.weaponShootRates['machinegun'] = Math.max(50, (scene.weaponShootRates['machinegun'] ?? 100) - 20) } },
    ],
  }
  const result: any[] = []
